...
import { getModelDisplayNameFromCatalog, getModelImageSupport, getModelImageSupportMessage } from '../utils/modelCapabilities';
...
export class ChatSidebarProvider implements vscode.WebviewViewProvider {
    ...
    private async handleGetCurrentProvider(webview: vscode.Webview) {
        ...
        const routingState = this.getImageInputRoutingState(currentModel || undefined);
        ...
        webview.postMessage({
            ...
            effectiveProvider: routingState.effectiveProvider,
            model: currentModel || defaultModel,
            supportsImageInput: routingState.supportsImageInput,
            imageInputSupportMessage: routingState.imageInputSupportMessage
        });
    }
    ...
    private async handleChangeProvider(model: string, webview: vscode.Webview) {
        try {
            ...
            await config.update('llmProvider', 'claude-api', vscode.ConfigurationTarget.Global);
            ...
            const routingState = this.getImageInputRoutingState(model);
            webview.postMessage({
    ...
    private getModelDisplayName(model: string): string {
        ...
        return getModelDisplayNameFromCatalog(model) || modelNames[model] || model;
    }

    private getImageInputRoutingState(modelId?: string): {
        effectiveProvider: string;
        supportsImageInput: boolean;
        imageInputSupportMessage: string;
    } {
        const config = vscode.workspace.getConfiguration('superdesign');
        ...
        const usesClaudeCode = aiModelProvider === 'claude-code' || llmProvider === 'claude-code';
        const modelSupport = getModelImageSupport(modelId);

        return {
            effectiveProvider: usesClaudeCode ? 'claude-code' : aiModelProvider,
            supportsImageInput: !usesClaudeCode && modelSupport !== 'unsupported',
            imageInputSupportMessage: usesClaudeCode
                ? 'Image understanding is unavailable while Superdesign is routed through Claude Code. Switch the main LLM provider to claude-api, then choose an Anthropic, OpenAI, or OpenRouter model.'
                : getModelImageSupportMessage(modelId)
        };
    }
}
