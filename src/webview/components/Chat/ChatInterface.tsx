...
import { listImageMetadata } from '../../../utils/chatContent';
import { ChatConversationSummary } from '../../hooks/useChat';
...
function getContextImages(context: CurrentContextState | null): ContextImage[] {
    if (!context || (context.type !== 'image' && context.type !== 'images')) {
        return [];
    }
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ layout, vscode }) => {
    ...
    const [supportsImageInput, setSupportsImageInput] = useState<boolean>(true);
    const [imageInputSupportMessage, setImageInputSupportMessage] = useState<string>('');
    ...
    const [pendingImages, setPendingImages] = useState<ContextImage[]>([]);
    ...
    useEffect(() => {
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if (message.command === 'currentProviderResponse') {
                ...
                setSupportsImageInput(message.supportsImageInput !== false);
                setImageInputSupportMessage(message.imageInputSupportMessage || '');
    ...
    const requestImagePreview = useCallback((filePath?: string) => {
        if (!filePath) {
            return;
        }
    ...
    const showImageInputUnsupported = () => {
        vscode.postMessage({
            command: 'showError',
            data: imageInputSupportMessage || 'Image understanding is not available with the current provider configuration.'
        });
    };
            ...
            const handleMessage = (event: MessageEvent) => {
                ...
                if (message.command === 'canvasStatusResponse') {
                    ...
                    setPendingImages(prev => [...prev, {
                        ...
                        previewData: message.data.previewData,
                        mimeType: message.data.mimeType
                    }]);
                    if (message.data.fullPath && message.data.previewData) {
                        setImagePreviews(prev => ({
                            ...prev,
                            [message.data.fullPath]: {
                                status: 'loaded',
                                dataUrl: message.data.previewData
                            }
                        }));
                    }
                    ...
                    setUploadingImages(prev => prev.filter(name => name !== message.data.originalName));
                } else if (message.command === 'base64ImageResponse') {
                    setImagePreviews(prev => ({
                        ...prev,
                        [message.filePath]: message.error
                            ? { status: 'error' }
                            : { status: 'loaded', dataUrl: message.base64Data }
                    }));
    ...
    useEffect(() => {
        ...
        const frameId = window.requestAnimationFrame(() => {
            const behavior: ScrollBehavior = isLoading ? 'auto' : chatHistory.length > 0 ? 'smooth' : 'auto';
    ...
    }, [chatHistory, isLoading, showWelcome, currentContext, uploadingImages.length]);
    ...
    const handleRemoveContextImage = useCallback((fullPath: string) => {
        setCurrentContext(prev => {
            const existingImages = getContextImages(prev).filter(image => image.fullPath !== fullPath);
            if (existingImages.length === 0) {
                return null;
            }

            return {
                fileName: existingImages.map(image => image.fullPath).join(', '),
                type: existingImages.length === 1 ? 'image' : 'images',
                images: existingImages
            };
        });
    }, []);

    const renderImageChip = useCallback((image: ContextImage, options?: { removable?: boolean; compact?: boolean }) => {
        const removable = options?.removable ?? false;
        const compact = options?.compact ?? false;
        const previewState = imagePreviews[image.fullPath];
        const previewData = image.previewData || previewState?.dataUrl;

        return (
            <div
                key={image.fullPath}
                className={`image-context-chip ${compact ? 'image-context-chip--compact' : ''}`}
                onMouseEnter={() => requestImagePreview(image.fullPath)}
            >
                <span className="image-context-chip__icon">🖼️</span>
                <span className="image-context-chip__label">{image.fileName || getDisplayFileName(image.fullPath)}</span>
                {removable && (
                    <button
                        className="image-context-chip__remove"
                        onClick={(event) => {
                            event.stopPropagation();
                            handleRemoveContextImage(image.fullPath);
                        }}
                        title={`Remove ${image.fileName || getDisplayFileName(image.fullPath)}`}
                    >
                        ×
                    </button>
                )}
                <div className="image-context-preview">
                    {previewData ? (
                        <img
                            className="image-context-preview__image"
                            src={previewData}
                            alt={image.fileName || getDisplayFileName(image.fullPath)}
                        />
                    ) : previewState?.status === 'error' ? (
                        <div className="image-context-preview__status">Preview unavailable</div>
                    ) : (
                        <div className="image-context-preview__status">Loading preview...</div>
                    )}
                </div>
            </div>
        );
    }, [handleRemoveContextImage, imagePreviews, requestImagePreview]);
        ...
        if (inputMessage.trim()) {
            ...
            if (currentContext && (currentContext.type === 'image' || currentContext.type === 'images')) {
                ...
                try {
                    ...
                    const contextImages = getContextImages(currentContext);
                    const imagePaths = contextImages.length > 0
                        ? contextImages.map(image => image.fullPath)
                        : (currentContext.type === 'images'
                            ? currentContext.fileName.split(', ')
                            : [currentContext.fileName]);
                    
                    // Store image references and let the extension resolve binary data on demand
                    for (const imagePath of imagePaths) {
                        try {
                            ...
                            console.log('📎 Added image reference to message:', imagePath, 'MIME:', mimeType);
                        } catch (error) {
    ...
    const handleDeleteConversation = (conversation: ChatConversationSummary) => {
        if (isLoading) {
            return;
        }
    ...
    useEffect(() => {
        if (uploadingImages.length === 0 && pendingImages.length > 0) {
            setCurrentContext(prev => {
                const mergedImages = new Map<string, ContextImage>();

                for (const image of getContextImages(prev)) {
                    mergedImages.set(image.fullPath, image);
                }
                ...
                const nextImages = Array.from(mergedImages.values());

                return {
                    fileName: nextImages.map(image => image.fullPath).join(', '),
                    type: nextImages.length === 1 ? 'image' : 'images',
                    images: nextImages
                };
            });
    ...
    }, [uploadingImages.length, pendingImages]);
    ...
    const renderChatMessage = (msg: ChatMessage, index: number) => {
        ...
        const imageMetadata = listImageMetadata(msg.content);
                ...
                <div className="chat-message__content">
                    {(msg.role === 'assistant') ? (
                        ...
                        (() => {
                            ...
                            if (msg.role === 'user' && imageMetadata.length > 0) {
                                return (
                                    <>
                                        <div className="message-context-display message-context-display--images">
                                            <div className="message-context-display__header">
                                                <span className="context-icon">🖼️</span>
                                                <span className="context-text">
                                                    {imageMetadata.length === 1 ? '1 image attached' : `${imageMetadata.length} images attached`}
                                                </span>
                                            </div>
                                            <div className="message-context-display__chips">
                                                {imageMetadata.map((image, imageIndex) => renderImageChip({
                                                    fileName: image.fileName || `image-${imageIndex + 1}`,
                                                    originalName: image.fileName || `image-${imageIndex + 1}`,
                                                    fullPath: image.filePath || image.fileName || `image-${imageIndex + 1}`
                                                }, { compact: true }))}
                                            </div>
                                        </div>
                        ...
                        <div className="conversation-history-menu">
                            ...
                            <div className="conversation-history-menu__list">
                                {conversations.map(conversation => (
                                    <div
                                        key={conversation.id}
                                        className={`conversation-history-item ${conversation.id === activeConversationId ? 'is-active' : ''}`}
                                    >
                                        <button
                    ...
                    <div className="chat-input-wrapper">
                        ...
                        {currentContext ? (
                            currentContext.type === 'image' || currentContext.type === 'images' ? (
                                <div className="context-display context-display--images">
                                    <div className="context-display__header">
                                        <span className="context-icon">🖼️</span>
                                        <span className="context-text">
                                            {getContextImages(currentContext).length === 1
                                                ? '1 image attached'
                                                : `${getContextImages(currentContext).length} images attached`}
                                        </span>
                                        <button 
                                    ...
                                    <div className="context-image-list">
                                        {getContextImages(currentContext).map(image => renderImageChip(image, { removable: true }))}
                                    </div>
                                </div>
                            ) : (
                                <div className="context-display">
                                    <span className="context-icon">📄</span>
                                    <span className="context-text">
                                        Context: {getDisplayFileName(currentContext.fileName)}
                                    </span>
                                    <button 
                                        className="context-clear-btn"
                                        onClick={() => setCurrentContext(null)}
                        ...
                        {imageInputSupportMessage && (
                            <div className="context-display">
                                <span className="context-icon">!</span>
                                <span className="context-text">{imageInputSupportMessage}</span>
                            </div>
                        )}
                        ...
                        <div className="input-controls">
                            ...
                            <div className="input-actions">
                                <button 
                                    ...
                                    disabled={isLoading || showWelcome}
                                    title={supportsImageInput ? 'Attach images' : 'Image input unavailable with the current provider'}
                                >
                                    <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
