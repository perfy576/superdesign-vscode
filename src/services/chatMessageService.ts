...
export class ChatMessageService {
    ...
    async handleChatMessage(message: any, webview: vscode.Webview): Promise<void> {
        try {
            ...
            const hydratedChatHistory = await this.hydrateImageMessages(chatHistory);
            
            console.log('========chatHistory', chatHistory);

            Logger.info(`Chat message received with ${chatHistory.length} history messages`);
            Logger.info(`Latest message: ${summarizeContent(messageContent)}`);
            ...
            if (typeof messageContent !== 'string' && Array.isArray(messageContent)) {
                ...
                messageContent.forEach((part, index) => {
                    if (part.type === 'text') {
                        ...
                        const imageSize = typeof imageData === 'string' ? imageData.length : 'binary';
                        Logger.info(`  [${index}] image: ${part.mimeType || 'unknown type'} (${imageSize} chars)`);
                    }
                });
            } else {
                Logger.info(`Simple text content: ${summarizeContent(messageContent).substring(0, 100)}...`);
            }
            ...
            hydratedChatHistory.forEach((msg, index) => {
                const content = summarizeContent(msg.content) || '[complex content]';
            ...
            let response: any[];
            const effectiveChatHistory = hydratedChatHistory.length > 0
                ? hydratedChatHistory
                : [{
                    role: 'user',
            ...
            if (effectiveChatHistory.length > 0) {
                ...
                this.outputChannel.appendLine(`Using conversation history with ${effectiveChatHistory.length} messages`);
                ...
                const latestTextMessage = extractTextFromContent(messageContent) || latestMessage;
    ...
    private async hydrateImageMessages(chatHistory: CoreMessage[]): Promise<CoreMessage[]> {
        return Promise.all(chatHistory.map(async message => {
            ...
            return {
                ...message,
                content: await this.hydrateImageContent(message.content)
            };
        }));
    }

    private async hydrateImageContent(content: CoreUserMessage['content']): Promise<CoreUserMessage['content']> {
        ...
        return Promise.all(content.map(async part => {
            ...
            const filePath = typeof superdesign?.filePath === 'string' ? superdesign.filePath : undefined;
            ...
            try {
                ...
                const mimeType = part.mimeType || mime.lookup(filePath) || 'image/png';
                ...
                Logger.warn(`Failed to hydrate image ${filePath}: ${error}`);
                return {
                    ...part,
                    image: '[missing-image]'
                };
