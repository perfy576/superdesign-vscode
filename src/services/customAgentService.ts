    ...
    private async setupWorkingDirectory(): Promise<void> {
        try {
            ...
            if (workspaceRoot) {
                ...
                this.outputChannel.appendLine(`Ensuring .superdesign directory exists at: ${superdesignDir}`);
...
`;}
    ...
    async query(
        ...
        if (aiModelProvider === 'claude-code' || llmProvider === 'claude-code') {
            ...
            if (historyHasImages(conversationHistory)) {
                throw new Error('Image input is not supported when Superdesign routes through Claude Code. Switch the main LLM provider to claude-api and use an Anthropic, OpenAI, or OpenRouter model.');
            }
            
            // Convert conversation history to prompt for ClaudeCodeService
            let queryPrompt = '';
            if (conversationHistory && conversationHistory.length > 0) {
                queryPrompt = serializeConversationForTextOnlyProvider(conversationHistory);
            } else if (prompt) {
                queryPrompt = prompt;
            } else {
                throw new Error('Either prompt or conversationHistory must be provided');
